...
describe('renewTransientGenerationDelegation', () => {
  ...
  it('refuses to mint when the document anchors no ladder VM of this credential', async () => {
    // The account the transient login's fallback serves: a document
    // anchoring no ladder VM of this credential, what a credential
    // retirement's strike leaves. A delegation signed by that VM would
    // authorize nothing and would poison the annex log for every later
    // visit.
    vi.mocked(verifyAccountLog).mockResolvedValue({
      did: ACCOUNT_DID,
