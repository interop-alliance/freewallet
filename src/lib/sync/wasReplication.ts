 ...
 * (`reSync()` / `cancel()`).
 *
 * No React, no Freewallet, no `@interop/was-client` imports -- only RxDB (the
 * replication engine being wrapped) and the injected port. This is the seam that
 * makes extraction to a standalone `was-rxdb-replication` library a repackaging
