...
A configured storage server must speak WAS v0.5 (was-teaching-server 0.33.0 or
newer).

| Variable                       | Default                                    | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------ | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PORT`                         | _(Vite default)_                           | Used by Docker, Dokku, etc. Automatically handled by Vite.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `VITE_WAS_SERVER_URL`          | _(none)_                                   | The remote [WAS](https://w3c-ccg.github.io/wallet-attached-storage-spec/) server's Spaces Repository URL, e.g. `https://host/spaces/` (`https://host/was/spaces/` on a sub-path mount). Service discovery starts from it, and the server base URL is derived as its parent (trailing slashes are ignored; a value that is not an absolute URL is refused at startup). A server that advertises a different Spaces Repository URL is refused at discovery. **Setting this attaches a remote replica: local collections replicate to it in the background.** If not provided, the app runs in a local-only mode (storing VCs in browser IndexedDB). |
...
| `VITE_CORS_PROXY_URL`          | `<WAS>/api/cors` or `https://corsproxy.io` | CORS proxy base URL for every cross-origin fetch the wallet makes on a user-supplied URL -- pasting a VC URL into the Add Credential box, and the `oidf` issuer-registry lookups during verification (a `dcc-legacy` registry file and the registries list go direct, retrying through the proxy if that is blocked). The target is appended as `?url=`. Defaults to the WAS server's `/api/cors` facet when a WAS server is configured, otherwise `https://corsproxy.io`; unset means the fetch goes direct.                                                                                                                                     |
